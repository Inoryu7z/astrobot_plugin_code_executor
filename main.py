...
class CodeExecutorPlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig = None):
        ...
        # 用户可见输出模式: silent / summary / verbose
        self.output_visibility = self.config.get("user_output_visibility", "summary")
        # 跨容器路径映射
        self.astrbot_shared_path = self.config.get("astrbot_container_path", "/AstrBot/data/napcat_shared")
        self.napcat_shared_path = self.config.get("napcat_container_path", "/app/napcat_shared")
        # 文件发送重试
        self.send_retry_count = self.config.get("file_send_retry_count", 2)
        self.send_retry_delay = self.config.get("file_send_retry_delay", 0.5)
    ...
    async def _send_user_message(self, event: AstrMessageEvent, message, level: str = "summary"):
        """
        根据用户可见输出模式过滤消息发送。
        level: "debug" - 仅verbose模式可见; "summary" - summary和verbose模式可见; "always" - 始终可见
        """
        if self.output_visibility == "silent":
            if level != "always":
                return
        elif self.output_visibility == "summary":
            if level == "debug":
                return
        # verbose 模式下全部发送
        await event.send(message)

    def _map_file_path(self, file_path: str) -> List[str]:
        """
        根据跨容器路径映射配置，生成所有可能的文件路径列表。
        原路径 + 映射路径去重。
        """
        paths = [file_path]
        if self.astrbot_shared_path and self.napcat_shared_path:
            mapped = file_path.replace(self.astrbot_shared_path, self.napcat_shared_path)
            if mapped != file_path:
                paths.append(mapped)
        return paths

    async def _send_file_with_retry(self, file_path: str, event: AstrMessageEvent) -> tuple:
        """
        带自动重试和跨容器路径映射的文件发送。
        返回 (success: bool, file_name: str)
        中间过程仅记录日志，不发送用户消息。
        """
        file_name = os.path.basename(file_path)
        paths_to_try = self._map_file_path(file_path)
        
        for attempt in range(self.send_retry_count + 1):
            for p in paths_to_try:
                try:
                    # 尝试本地路由发送
                    if self.enable_local_route_sending and self.enable_webui and self.webui:
                        success = await self._send_file_via_local_route(p, event)
                        if success:
                            logger.info(f"文件发送成功(本地路由): {file_name}")
                            return True, file_name
                        logger.debug(f"本地路由发送失败，尝试其他发送方式: {p}")

                    # AstrBot原生发送
                    is_image = any(
                        file_name.lower().endswith(ext) for ext in ['.png', '.jpg', '.jpeg', '.gif', '.bmp'])
                    if is_image:
                        logger.info(f"正在以图片形式发送: {p}")
                        await event.send(MessageChain().file_image(p))
                    else:
                        chain = [Comp.File(file=p, name=file_name)]
                        await event.send(event.chain_result(chain))
                    logger.info(f"文件发送成功: {file_name}")
                    return True, file_name
                except Exception as e:
                    logger.debug(f"文件发送尝试失败 (路径={p}, 第{attempt+1}次): {e}")
                    continue
        ...
        # 所有重试都失败，尝试base64作为最后手段
        try:
            base64_success = await self._send_file_via_base64(file_path, event)
            if base64_success:
                logger.info(f"文件发送成功(base64): {file_name}")
                return True, file_name
        except Exception as e:
            logger.debug(f"base64发送也失败: {e}")
        
        logger.error(f"文件发送最终失败: {file_name} (已重试{self.send_retry_count}次)")
    ...
    async def execute_python_code(self, event: AstrMessageEvent, code: str, description: str = "") -> str:
        ...
        **步骤2：将文件路径添加到 FILES_TO_SEND**
        ...
        if not self.allow_all_users and event.role != "admin":
            await self._send_user_message(event, MessageChain().message("❌ 你没有权限使用此功能！"), level="always")
        ...
        if event.role != "admin" and self.non_admin_safety_enabled:
            ...
            if matched:
                ...
                await self._send_user_message(event, MessageChain().message(text), level="always")
                return text
        ...
        try:
            ...
            if result["success"]:
                ...
                text_response = "\n".join(response_parts)
                await self._send_user_message(event, MessageChain().message(text_response), level="summary")
                ...
                if result["file_paths"]:
                    ...
                    for file_path in result["file_paths"]:
                        if not os.path.exists(file_path) or not os.path.isfile(file_path):
                            ...
                            continue
                        success, file_name = await self._send_file_with_retry(file_path, event)
                        if success:
                            success_count += 1
                        else:
                    ...
                    if failed_files:
                        await self._send_user_message(
                            event, 
                            MessageChain().message(f"❌ 部分文件发送失败: {', '.join(failed_files)}"), 
                            level="summary"
                        )
                    
                    # LLM上下文只记录简洁的统计信息
                    if success_count > 0:
                        llm_context_parts.append(f"📄 已发送 {success_count} 个文件")
                ...
                await self._send_user_message(event, MessageChain().message(error_msg), level="summary")
